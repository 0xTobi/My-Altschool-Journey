    ...
    const firstFour = userInput.slice(0, 4);
    ...
    let phoneNo = String(userInput.value);


    // to check the validity of the number before validating the number
    let validity = false;

    if (phoneNo.includes("+234") && phoneNo.length === 14) {
      validity = true;
    }

    if (phoneNo.includes("+2340") && phoneNo.length === 15) {
      validity = true;
    }
    ...
    // // Second loop goes over each element in the patterns array of each object.
